...
import time
import ConfigParser
render = web.template.render('img/')
...
class myThread (threading.Thread):   #继承父类threading.Thread
    ...
    def run(self):                   #把要执行的代码写到run函数里面 线程在创建后会直接运行run函数 
        ...
        if self.name == 'Thread-web':
            run4web()
        elif self.name == 'Thread-listener':
            i = 0
            while 1:
                try:
                    lines = open('uuid_new.txt','r').readlines()
                except:
                    f = open('uuid_new.txt','w')
                    f.close()
                    continue
                if len(lines) == 0:
                    time.sleep(5)
                    print '%s:文件为空' % self.name
                    continue
                print 'start-----------:%s' % lines
                bot = WXBot()
                print lines[i]
                file = open('uuid_now.txt','w')  # @ReservedAssignment
                file.write(lines[i][:12])
                file.close()
                myThread(i, "Thread-"+str(i), 2).start()
                i+=1
                if i == len(lines):
                    i=0
                    open('uuid_new.txt','w').write('')
        else:
            bot = WXBot()
            bot.uuid['uuid'] = open('uuid_now.txt','r').read()
            bot.run()
...
class WXBot:
    ...
    def GET(self, name):
        ...
        # 将uuid写入文件,使用追加写，一次请求会产生两个
        file = open('uuid_new.txt','a')  # @ReservedAssignment
    ...
    def get_group_member_name(self, gid, uid):
        ...
        :param gid: 群id
        :param uid: 群聊成员id
        :return: 名称信息，类似 {"display_name": "test_user", "nickname": "start", "remark_name": "for_test" }
        """
...
#             self.send_msg_by_uid(u'hi', msg['user']['id'])
    ...
    def wait4login(self):
        ...
        while retry_time > 0:
            url = LOGIN_TEMPLATE % (tip, self.uuid.get('uuid'), int(time.time()))
